var cradle = require('cradle')
  , LatLon = require('./geo/latlon').LatLon
  , geo = require('./geo/geo');
...
Geocradle.prototype.loadPositions = function (viewName,query,callback) {
  ...
  if (query.bbox)
    ...
    options.bbox = this._retrieveBbox(query);
  else 
  ...
  this.db.spatial(viewName, options, function (err, doc) {     
    if (doc) {
      console.dir(doc);
    }
    else {
...
// retrieve bbox from a given point 
// upper-left corner
// lower-right corner
Geocradle.prototype._retrieveBbox = function(point){
  var p1 = new LatLon(geo.parseDMS(point.latitude), geo.parseDMS(point.longitude));
  
  // in case point distance is defined set distance as Math.sqrt( 2 * Math.pow(distance,2))
